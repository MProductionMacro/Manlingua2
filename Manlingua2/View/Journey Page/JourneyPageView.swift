                                ...
                                ScrollView(.vertical, showsIndicators: false) {
                                    VStack(spacing: 24) {
                                        ForEach(story.subChapter, id: \.self){ subChapter in
                                            if subChapter.id <= 2{
                                                SubChapterCard(labelImage: .haidilaoChapter, chapter: "Bagian \(subChapter.id)", title: "\(subChapter.title)", pinyin: "\(subChapter.pinyin)", hanzi: "\(subChapter.hanzi)" ,isLocked: false).padding(.horizontal)
                                            }
                                            else{
                                                SubChapterCard(labelImage: .haidilaoChapter, chapter: "Bagian \(subChapter.id)", title: "\(subChapter.title)", pinyin: "\(subChapter.pinyin)", hanzi: "\(subChapter.hanzi)" ,isLocked: true)
                                                    .padding(.horizontal)
                                            }
                                        }
                                    }
